...
author: 
  ...
  - Darren James
date: 2022-06-19
description: An introduction to some of the datasets used in these lessons.
execute:
  eval: true
  echo: true
---
...
dataframe. You must also provide an API access key at the end of the URL, so
the URL will look something like `https://pasta.lternet.edu/<path_to_data_file>?key=<my_API_key>`.
To get an API access key, visit the [EDI authentication portal](https://auth.edirepository.org).
...
anpp.annual <- read_csv(input_file, na=c('NA', '', '.'))
```
