...
author: Greg Maurer
date: 2022-09-01
description: A fairly simple Git and GitHub tutorial
execute:
  eval: true
  echo: true
---

This is a simple git and GitHub tutorial to teach you the basics of working with version control.
...
* For example, [this tutorial](https://github.com/jornada-im/jeds/tree/main/docs/lessons) is in a repository on GitHub.
...
#| label: mkdir_GitHub
#| eval: true
mkdir -p GitHub # -p option is "if it doesn't exist already"
```
...
Now lets make those changes. Look inside the `docs/lessons` folder and find the
`learner_changes.md` file.

```{bash}
#| label: find_jeds2_file
#| eval: true
ls docs/lessons/
``` 
...
```{bash}
#| label: change_jeds2_repo
#| include: false
echo "This repository does this" >> docs/lessons/learner_changes.md
echo "This repository does that" >> docs/lessons/learner_changes.md
```
```{bash}
#| label: commit_jeds2_file
#| eval: true
git add docs/lessons/learner_changes.md
...
`docs/lessons/learner_changes.md`. If we want to contribute the changes in our new
...
you can now view all files in `my_changes`. If you like, open `docs/lessons/learner_changes.md`
